...
export function foldSessionUsage(events, pricingLookup) {
        ...
        if (ev.type === "assistant/message") {
            ...
            if (d?.usage) {
                if (d.usage.inputTokens)
                    promptTokens = Math.max(promptTokens, d.usage.inputTokens);
                if (d.usage.outputTokens)
                    completionTokens += d.usage.outputTokens;
                if (d.usage.reasoningTokens)
                    reasoningTokens = (reasoningTokens ?? 0) + d.usage.reasoningTokens;
                if (d.usage.cacheReadTokens)
                    cacheReadTokens = (cacheReadTokens ?? 0) + d.usage.cacheReadTokens;
                if (d.usage.cacheCreationTokens)
                    cacheCreationTokens = (cacheCreationTokens ?? 0) + d.usage.cacheCreationTokens;
                if (d.usage.costUsd)
                    costUsd = (costUsd ?? 0) + d.usage.costUsd;
            }
    ...
    if (promptTokens > 0 && totalEstimatedPrompt > 0) {
        ...
        if (promptTokens === 0)
            promptTokens = totalEstimatedPrompt;
    }
